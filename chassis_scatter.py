# -*- enconding: utf-8 -*-
# @ModuleName: beam_scatter
# @Function: plot scatter for beam
# @Author: Yanzhan Chen
# @Time: 2021/4/30 9:30
...
class othermodel():
    ...
    def ourmodel(self):
        ...
        for i in range(4):
            ...
            # model = SVR()
            model.fit(X_train, y_train)
            # save_name = './model/'+ 'hood' + target_list[i] +'.pkl'
            # joblib.dump(model,save_name)

            y_true = y_train[0:622]
            y_pred = model.predict(X_train[0:622,:])
            print(y_true[-3:])
            print(y_pred[-3:])
            ...
            plt.scatter(y_true, y_pred, s=7, c='#00aa00ff',label='Data Points', alpha=1)
